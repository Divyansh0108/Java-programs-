...
public class PrefixSumApproach {
    ...
    // time complexity : O(1)
    // space complexity : O(1)
    public static int sumRegion(int[][] arr, int r1, int c1, int r2, int c2){ // time complexity : O(1)
    ...
    public static int findSumMatrix(int[][] arr, int r1, int c1, int r2, int c2){ // time complexity : O(m * n)
        int sum = 0;
        for(int i=r1; i<=r2; i++){
            for(int j=c1; j<=c2; j++){
                sum += arr[i][j]; // O(1)
    ...
    public static void main(String[] args) { // time complexity : O(m * n)
        ...
        prefixSumMatrix(arr); // O(m * n)
        int result = sumRegion(arr, r1, c1, r2, c2); // O(1)
        System.out.println("Sum of elements of given rectangle is: "+result);
    }
}
// PrefixSumApproach.java
// Displaying PrefixSumApproach.java.